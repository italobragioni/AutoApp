...
import { formatWeight } from "@/lib/niches/serralheria/pricing";
import { baseUnitLabel } from "@/lib/niches/serralheria/options";

// Visualizacao profissional do orcamento de Serralheria (server component).
...
function measures(spec: ItemSpec): string {
  const parts: string[] = [];
  if (spec.widthMm && spec.heightMm) parts.push(`${spec.widthMm}×${spec.heightMm} mm`);
  if (spec.lengthMm) parts.push(`C: ${spec.lengthMm} mm`);
  if (spec.weightKg) parts.push(`${spec.weightKg} kg/pç`);
  return parts.length ? parts.join(" · ") : "-";
}

export function SteelQuoteDocument({ quote }: { quote: SteelQuoteDocData }) {
  const sums = quote.items.reduce(
    (acc, it) => {
      const b = (it.breakdown ?? {}) as ItemBreakdown;
      acc.material += b.materialCents ?? 0;
      acc.labor += b.laborCents ?? 0;
      acc.paint += b.paintCents ?? 0;
      acc.weight += b.weightKgTotal ?? 0;
      return acc;
    },
    { material: 0, labor: 0, paint: 0, weight: 0 },
  );
  ...
  return (
    <div className="rounded-xl border border-surface-border bg-white p-6 text-ink print:border-0 print:p-0">
        ...
        <table className="w-full min-w-[720px] text-xs">
          <thead className="border-b border-surface-border text-left text-ink-faint">
            <tr>
              ...
              <th className="py-2 pr-2 text-right font-medium">Pintura</th>
          ...
          <tbody>
              ...
              return (
                <tr key={it.id} className="border-b border-surface-border align-top">
                  ...
                  <td className="py-2 pr-2 text-ink-soft">
                    {spec.serviceTypeLabel ?? "-"}
                    {spec.baseUnit ? <span className="block text-ink-faint">{baseUnitLabel(spec.baseUnit)}</span> : null}
                  </td>
                  ...
                  <td className="py-2 pr-2 text-right text-ink-soft">
                    {typeof b.weightKgTotal === "number" && b.weightKgTotal > 0 ? formatWeight(b.weightKgTotal) : "-"}
                  </td>
                  <td className="py-2 pr-2 text-right text-ink-soft">{formatCents(b.materialCents ?? 0)}</td>
                  <td className="py-2 pr-2 text-right text-ink-soft">{formatCents(b.laborCents ?? 0)}</td>
                  <td className="py-2 pr-2 text-right text-ink-soft">{formatCents(b.paintCents ?? 0)}</td>
            ...
            {quote.items.length === 0 ? (
              <tr>
                <td colSpan={9} className="py-6 text-center text-ink-faint">
      ...
      <div className="mt-4 flex flex-wrap justify-between gap-4">
        <div className="text-xs text-ink-soft">
          {sums.weight > 0 ? <p>Peso total estimado: {formatWeight(sums.weight)}</p> : null}
          {quote.deliveryTime ? <p className="mt-2"><span className="font-semibold text-ink">Prazo estimado:</span> {quote.deliveryTime}</p> : null}
        ...
        <div className="w-full max-w-xs space-y-1 text-sm">
          ...
          <Row label="Pintura" value={formatCents(sums.paint)} />
          <Row
            label="Custos adicionais"
            value={formatCents(quote.installationCents + quote.travelCents + quote.otherCents)}
          />
